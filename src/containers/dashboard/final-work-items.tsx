...
const greet=()=> {
    ...
    const fetchData = useCallback(async ({ pageSize, pageIndex }: any) => {
        ...
        const adminPrimaryId = Number(LocalStorageManager.getAdminPrimaryId());
        const { data } = await finalService.getFinalsByUserId(vfromdate,vtodate, adminPrimaryId,'final',statusValue,istatkal);
       // if (data.length > 0) {
    ...
    const getDoctorDetailsByMobile = async () => {
        try {
            ...
            if (mobileNo.length === 10) {
                ...
                const { data } = await finalService.getFinalsByMobileNoByUserId(formData);
                if (fetchId === fetchIdRef.current) {
                    const startRow = pageSize * pageIndex
                    const endRow = startRow + pageSize
                    if (data != undefined) {
