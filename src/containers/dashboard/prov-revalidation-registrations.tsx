...
const greet=()=> {
            ...
            Cell: (cell: any) => (
                <>
                    <a href="javascript:void(0);" onClick={() =>
                        {
                        setShowComponent(false);  
                        toggleComponent(cell.data[Number(cell.row.id)].prvId,cell.data[Number(cell.row.id)].doctorPrimaryId, cell.data[Number(cell.row.id)].assignmentId);
                        }}>Proceed</a>
                </>
            )
        },
            ...
            Cell: (cell: any) => (
                ...
                {cell.data[Number(cell.row.id)].assignedUserName ===null && cell.data[Number(cell.row.id)].status ==='pen' ? 
                    <input  type="checkbox" id={cell.row.id} checked={selected[cell.row.id]} onClick={async (e:any) => {
                        ...
                        const { data } = await assignmentService.getAssignMentBydoctorIdAssignType(cell.data[Number(cell.row.id)].doctor_id, 'revalidation',cell.data[Number(cell.row.id)].prvId);
    ...
    }, [showComponent,viewadditionalid]);
    ...
    const fetchData = useCallback(async ({ pageSize, pageIndex }: any) => {
        ...
        const { data } = await revalidationService.getRevalidationsByFilter(vfromdate,vtodate, statusValue,istatkal);
    ...
    const getDoctorDetailsByMobile = async () => {
        try {
            ...
            if (mobileNo.length === 10) {
               ...
               const { data } = await revalidationService.getRevalidationByMobileNo(formData);
               if (fetchId === fetchIdRef.current) {
                const startRow = pageSize * pageIndex
                const endRow = startRow + pageSize
               if(data!=undefined){
...
export default ProvRevalidationReg;
