...
const greet=()=> {
                ...
                <>
                    {cell.data[Number(cell.row.id)].assignedUserName === null && cell.data[Number(cell.row.id)].approval_status === 'pen' ?
                        <input type="checkbox" id={cell.row.id} checked={selected[cell.row.id]} onClick={async (e: any) => {
                            ...
                            const { data } = await assignmentService.getAssignMentBydoctorIdAssignType(cell.data[Number(cell.row.id)].doctor_id, 'final',cell.data[Number(cell.row.id)].finalPrimaryId);
    ...
    const getDoctorDetailsBydocId = async () => {
        try {
            const fetchId = ++fetchIdRef.current
            const pageSize = 10;
            const pageIndex = 0
            if (docId.length > 3) {
                ...
                alert("Please enter at least 4 characters of  docId");
            }
            ...
            <div className="container-fluid">
                ...
                <div className="input-group-text p-0">
                        ...
                        <button type="submit"
                            ...
                            <label htmlFor="" className='mb-2'>Doctor Id: </label>
                        <input type="text" className='fs-14 w-100' id="name" onBlur={(e) => setdocId(e.target.value)} placeholder='Enter DoctorId' />
                        <button type="submit"
                            disabled={disablebtn}
                            onClick={
                                getDoctorDetailsBydocId
                            } className='btn bi-search '> </button>
                    </div>     
