...
const AdditionalWorkItems = () => {
  ...
  const toggleComponent = useCallback(async (additionalId:any,docId:any,assignId:any) => {
    try {
            let newValue = additionalId  ? additionalId  : viewadditionalid;
            setViewadditionalId(newValue);
            setViewDocId(docId);
...
const greet=()=> {
            ...
            Cell: (cell: any) => (
                <>
                    <a href="javascript:void(0);" onClick={() =>
                        {
                        setShowComponent(false);  
                        toggleComponent(cell.data[Number(cell.row.id)].additionalPrimaryId,cell.data[Number(cell.row.id)].doctorPrimaryId, cell.data[Number(cell.row.id)].assignmentId);
                        }}>Proceed</a>
                </>
            )
        }
    ...
    }, [showComponent,viewadditionalid,fromdate,todate,istatkal]);
    ...
    const fetchData = useCallback(async ({ pageSize, pageIndex }: any) => {
        ...
        const adminPrimaryId = Number(LocalStorageManager.getAdminPrimaryId());
        const { data } = await additionalService.getQualificationsByUserId(vfromdate,vtodate,adminPrimaryId,'additional',statusValue,istatkal);
     //   if (data.length > 0) {
    ...
    const getDoctorDetailsByMobile = async () => {
        try {
            ...
            if (mobileNo.length === 10) {
                ...
                const { data } = await additionalService.getAddlQualifByMobileNoByUserId(formData);
                if (fetchId === fetchIdRef.current) {
                    const startRow = pageSize * pageIndex
                    const endRow = startRow + pageSize
                    if (data != undefined) {
    ...
    const resetInput = () => {
      ...
      const getDashboardDetails = useCallback(async (assignedUser:any) => {
        try {
          const { data } = await commonService.AdditionalStatusCnt(assignedUser);
                if (data) {
                  setdataList(data);
                }
        }
        ...
        <>
            <div className="container-fluid">
                ...
                <div className="container">
                    ...
                    <div className="col-sm">
                        {dataList&&<UserCard Pending={dataList.additionalPenCnt} Verified={dataList.additionalVerCnt}/>}
                    </div>
                </div>
            </div>
            ...
            {showComponent === true?<AdditionalRegView state={{ additionalPrimaryId:viewadditionalid , doctorPrimaryId: viewDocid, assignmentId:viewAssignid  }} greet={greet}></AdditionalRegView>:""}
        </>
    )
}
