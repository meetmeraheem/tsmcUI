...
const greet=()=> {
    ...
    const fetchData = useCallback(async ({ pageSize, pageIndex }: any) => {
        ...
        const { data } = await renewalService.getRenewalsByUserId(vfromdate, vtodate, adminPrimaryId, 'renewal',statusValue,istatkal);
       // if (data.length > 0) {
    ...
    const getDoctorDetailsByMobile = async () => {
        try {
            ...
            if (mobileNo.length === 10) {
                ...
                const { data } = await renewalService.getRenewalsByMobileNoByUserId(formData);
                if (fetchId === fetchIdRef.current) {
                    const startRow = pageSize * pageIndex
                    const endRow = startRow + pageSize
                    if (data != undefined) {
