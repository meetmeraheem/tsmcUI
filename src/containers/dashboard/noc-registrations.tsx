...
const Noc = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            Cell: (cell: any) => (
                <>
                    <Link to={'/admin/noc_reg_view'} state={{ finalPrimaryId: cell.data[0].finalPrimaryId, doctorPrimaryId: cell.data[0].doctorPrimaryId }}>Proceed</Link>
                </>
            )
        },
        {
    ...
    const fetchData = useCallback(async ({ pageSize, pageIndex }: any) => {
        ...
        const { data } = await nocService.getNocsByFilter(newdate, statusValue);
        // if (data.length > 0) {
