...
const EditNameChangeRegistration = () => {
    const navigate = useNavigate();
    const location = useLocation();
    const { changeofNamePrimaryId} = location.state
    const [changeOfName, setChangeOfName] = useState<changeOfNameType>();
    const [provisionalCertificate, setProvisionalCertificate] = useState<{ file?: File; error?: string } | null>(null);
   
    const initialFormData =  useMemo(
        () => ({
            doctor_id: 0,
            edu_cert1: changeOfName?.edu_cert1||'',
            extra_col3:'',
            reg_date:'',
            status:'',
            Gazette_No:'',
    ...
    const getChangeofNameDetails = useCallback(async () => {
        try {
            if (changeofNamePrimaryId) {
                const { data } = await changeofnameService.getNameChangeById(changeofNamePrimaryId);
                if (data) {
                    setChangeOfName({
                        doctor_id: data.doctor_id,
                        edu_cert1: data.gazzetNotificationDocument,
                        extra_col3:'',
                        reg_date:'',
                        status:'',
                        Gazette_No:'',
    ...
    const submitForm = useCallback(
        async (values: changeOfNameType) => {
            try {
                ...
                formData.append("changeofNameInfo", JSON.stringify(changeofNameInfo));
                if (provisionalCertificate?.file) {
                    formData.append("gazzetCertificate", provisionalCertificate?.file);
                }
               
                const { success } = await changeofnameService.editNameChange(changeofNamePrimaryId,formData);
                if (success) {
                 
                    Swal.fire({
                        title: "Success",
                        text: "Name Change Data  updated successfully",
                        icon: "success",
                ...
                Swal.fire({
                    text: "Name Change Data  updation failed",
                    icon: "error",
                    confirmButtonText: "OK",
                })
                console.log('error in Name Change Data  update', err);
            }
        },
                            ...
                            <div className="card shadow border-0 mb-4">
                                <div className="card-body">
                                    <div className="d-flex align-items-center justify-content-between">
                                        <h1 className='fs-22 fw-700 text-nowrap'>Edit Change Of Name</h1>
                                        <div>
                                        </div>
                                    </div>
                                    <hr />
...
export default EditNameChangeRegistration;
